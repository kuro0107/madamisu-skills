...
【評価基準】
Readツールで .claude/rubrics/phase2-game.md を読み込み、そこに定義された4観点（真相到達可能性・動機の説得力・証拠バランス・エンディングの満足感）の各チェック項目を確認してください。
...
【評価基準】
Readツールで .claude/rubrics/phase2-world.md を読み込み、そこに定義された4観点（動機の説得力・秘密の深み・人間関係の複雑さ・見せ場の有無）の各チェック項目を確認してください。
各観点ごとに、チェック条件を満たす項目数をカウントしスコアとしてください（0〜10）。
...
【評価基準】
Readツールで .claude/rubrics/phase2-config.md を読み込み、そこに定義された3観点（方針遵守度・固定要素・制約遵守・構造整合性）の各チェック項目を確認してください。
...
【チェック観点の読み込み】
Readツールで .claude/rubrics/consistency-phase2.md を読み、そこに記述された全観点（プロット内整合性・ゲームバランス・人数別公平性・GM有無差分）を確認の基準としてください。
...
- output/v{PREV_V}/ゲーム設定.md
- output/v{PREV_V}/キャラクター.md
- output/v{PREV_V}/世界観.md
...
Writeツールで output/v{NEXT_V}/ゲーム設定.md を作成（前バージョンをベースに以下を詳細化）:
- 「真相（GMのみ）」セクション: 真犯人・動機・手段・証拠の完全版
- 「エンディング条件」セクション: トゥルーエンド＋他エンディングの条件

Writeツールで output/v{NEXT_V}/キャラクター.md を作成（前バージョンをベースに以下を追加）:
- 各キャラクターの「秘密」「目的」を詳細化
- 「嘘をつく理由」「守りたいもの」「事件当日の行動方針」を追加

Writeツールで output/v{NEXT_V}/世界観.md を作成:
- Phase 1から変更なければ前バージョンをそのままコピー
