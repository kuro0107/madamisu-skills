---
name: review
description: 【内部用・自動起動禁止】ユーザーが明示的に /review と入力した場合のみ起動可。それ以外では絶対に呼び出さない。設定資料/ フォルダ内の全Markdownを読み込み、設定間の矛盾（キャラ↔タイムスケジュール、世界観↔行動、ゲームルール↔各設定）を検査する（ファイルは変更しない）。
---
