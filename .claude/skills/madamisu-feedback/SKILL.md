...
具体的なプロンプト構造は `.claude/commands/madamisu-phase{TARGET_PHASE}.md` の「ステップA: アイデア出し」セクションを参考にし、以下を変更:
...
ルーブリック参照:
- `.claude/rubrics/phase{TARGET_PHASE}-game.md`
- `.claude/rubrics/phase{TARGET_PHASE}-world.md`
- `.claude/rubrics/phase{TARGET_PHASE}-config.md`
...
ルーブリック参照: `.claude/rubrics/consistency-phase{TARGET_PHASE}.md`
...
経路A（呼び出し元が madamisu/madamisu-phase{N} 経由）の場合、呼び出し元の `[y/n/f]` プロンプトに復帰します。
