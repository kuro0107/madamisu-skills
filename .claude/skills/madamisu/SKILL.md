---
name: madamisu
description: マダミス（マーダーミステリー）シナリオの自動生成。元資料（フォルダまたは.md）を入力にPhase 0〜3を順次実行し、ゲーム設定・世界観・キャラクター・タイムスケジュールを output/v{N}/ に出力する。ユーザーが「マダミス作って」「シナリオ生成」「マーダーミステリー作成」と言ったとき、または /madamisu 起動時に使う。
---
